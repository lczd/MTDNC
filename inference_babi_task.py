...
from mtdnc.data.loader import DataLoader
from mtdnc.model.mann import MANN

"""
This script performs a inference with the given models of this repository on the bAbI task 1 or on 1-20. Please add the 
model name when calling the script. (dnc, mtdnc, biadnc, biadnc-all, biadnc-aug16-all)
"""

parser = argparse.ArgumentParser(description='Load model')
parser.add_argument('--model', type=str, default='MTDNC-all', help='model name')
model_name = parser.parse_args().model

expt_dir = "experiments/babi_task_mann_DESKTOP-1EE4K0O/github_example/session_1"

config_file = 'config.yml'
with open(os.path.join(expt_dir, config_file), 'r') as f:
    configs = yaml.load(f, yaml.Loader)  # load config from file
...
lastFileName = "model_dump.ckpt"
fileName = os.path.join(expt_dir, lastFileName)
...
with tf.Session(config=conf) as sess:
    ...
    for task in task_list:
        ...
        print('task: {}, all_corrects: {}, all_overall: {}.'.format(task, all_corrects, all_overall))
